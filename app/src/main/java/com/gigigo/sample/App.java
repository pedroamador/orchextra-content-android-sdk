...
public class App extends MultiDexApplication {
  ...
  @Override public void onCreate() {
    ...
    if (typeInitSDK == 1) Ocm.initialize(this);//  fake only set app

    if (typeInitSDK == 2) {
      Ocm.initialize(this, "117687721829", MainActivity.class);//fake init with pushes
    }

    //the style and the callback set in mainactivity when we have correct credentials or we want init ox on MainActivity and no
    //before
    //region old fake Init
    if (typeInitSDK == 3) {
